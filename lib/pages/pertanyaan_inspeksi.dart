...
class PertanyaanInspeksiPage extends StatefulWidget {

  final int surat_tugas_id;

  const PertanyaanInspeksiPage({Key? key, required this.surat_tugas_id}) : super(key: key);

  @override
  _PertanyaanInspeksiPageState createState() => _PertanyaanInspeksiPageState();

}

class _PertanyaanInspeksiPageState extends State<PertanyaanInspeksiPage>  with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () async {
      ...
      if (connectivityResult == ConnectivityResult.none) {
        ...
        if(kategoriSoalInspeksi.isEmpty){
          showDialog(
            context: context,
            builder: (BuildContext context) {
              return AlertDialog(
                title: const Text('Peringatan'),
                content: const Text('Data Pertanyaan Inspeksi tidak ada pada device anda, silahkan aktifkan koneksi internet untuk mengunduh data pertanyaan inspeksi'),
                actions: [
                  TextButton(
                    child: const Text('OK'),
                    onPressed: () {
        ...
        if(kategoriSoalInspeksi.length == 0){
            ...
            showDialog(
              ...
              builder: (BuildContext context) {
                return AlertDialog(
                  ...
                  content: const Text('Data berhasil di simpan pada mode offline'),
                  actions: [
                    TextButton(
                      onPressed: () {
                        Navigator.of(context).pop();
            ...
            dataPertanyaan.forEach((pertanyaan) async {
              ...
              if(pertanyaan['rekomendasi'] == null){
                ...
                dbHelper.getRekomendasiByPertanyaanId(pertanyaan['rekomendasi']['pertanyaan_inspeksi_id']).then((value) {
                  if (value == null) {
                    ...
                    print('data rekomendasi sudah ada');
                  }
                });
              ...
              dbHelper.getInspeksiPertanyaanBySubKategoriIdAndIdPertanyaan(pertanyaan['subkategori_inspeksi_id'], pertanyaan['id_pertanyaan'], pertanyaan['id']).then((value) {
                if (value == null) {
                  ...
                  print('data pertanyaan sudah ada');
                }
              });
              ...
              TextButton(
                onPressed: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => SelesaiInspeksiPage(surat_tugas_id: widget.surat_tugas_id)
                    ),
                  );
                },
                child: const Text('OK'),
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          ...
          Visibility(
            visible: pertanyaan.jawaban == 1 || pertanyaan.jawaban == 2,
            child: Container(
              child: Column(
                children: <Widget>[
                  ElevatedButton.icon(
                  ...
                  ElevatedButton.icon(
                    onPressed: () {
                      _saveJawabanPertanyaan(pertanyaan.id_inspeksi_pertanyaan, pertanyaan.jawaban);
                    },
                    icon: const Icon(
                      Icons.save
                    ),
                    ...
                    onPressed: () {
                      showDialog(
                        ...
                        builder: (BuildContext context) {
                          return AlertDialog(
                            title: const Text('Peringatan'),
                            content: const Text('Apakah anda yakin ingin menyimpan jawaban?'),
                            actions: [
                              TextButton(
                                onPressed: () {
                                  Navigator.of(context).pop();
                                  ...
                                  if (status == 'Online') {
                                    ...
                                    showDialog(
                                      ...
                                      builder: (BuildContext context) {
                                        return AlertDialog(
                                          title: const Text('Peringatan'),
                                          content: const Text('Anda tidak dapat menyimpan jawaban karena anda sedang offline'),
                                          actions: [
                                            TextButton(
                                              onPressed: () {
                                                Navigator.of(context).pop();
